...
import { AlignmentActions, BannedRangeTags } from "../config/types.js";
import { findStaleOverridePins, findWorkspaceOverrideDrift } from "./overrides.js";

describe("findStaleOverridePins", () => {
  it("warns when an override pin sits below a requested range", () => {
  ...
  it("errors when an override uses latest", () => {
    ...
    assert.equal(findings.length, 1);
    assert.equal(findings[0].severity, AlignmentActions.Error);
    assert.match(findings[0].message, new RegExp(BannedRangeTags.Latest));
  });

  it("warns when a workspace-only override pin sits below a requested range", () => {
    const findings = findStaleOverridePins(
      {},
      {
        packages: {
