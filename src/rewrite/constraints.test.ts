...
describe("rewriteConstraint", () => {
  it("rewrites exact pins to caret for same-major", () => {
    assert.equal(rewriteConstraint("1.2.3", UpgradeModes.SameMajor), "^1.2.3");
  });

  it("rewrites exact pins to gte for latest", () => {
    assert.equal(rewriteConstraint("1.2.3", UpgradeModes.Latest), ">=1.2.3");
  });

  it("uses the range minimum when the spec is already a range", () => {
    assert.equal(rewriteConstraint("^2.3.3", UpgradeModes.SameMajor), "^2.3.3");
    assert.equal(rewriteConstraint("^2.3.3", UpgradeModes.Latest), ">=2.3.3");
  });

  it("skips workspace and file specs", () => {
    assert.equal(shouldSkipSpec("workspace:*"), true);
    assert.equal(shouldSkipSpec("file:../lib"), true);
    assert.equal(rewriteConstraint("workspace:*", UpgradeModes.SameMajor), null);
  });
});
